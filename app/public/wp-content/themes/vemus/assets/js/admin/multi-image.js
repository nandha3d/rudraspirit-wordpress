;(function($) {
        ...
        $('a.multi-images-upload').each(function() {
            ...
            button.on('click', function(evt) {
                ...
                file_frame.on('select', function(evt) {
                    ...
                    wp.customize.instance( inputId ).set( value);
                });
                // open the file frame
                file_frame.open();
    ...
    var multiImagesRemove = function() {
        $('a.multi-images-remove').each(function() {
            ...
            button.on('click', function(evt) {
                ...
                wp.customize.instance( input ).set('');
        ...
        $('input.multi-images-control-input').each(function() {
            ...
            input.on('updateThumbnails', function(evt, args) {
                ...
                for (var i = urls.length - 1; i >= 0; i--) {
                    ...
                    // Add a delete button to each thumbnail
                    var deleteBtn = $('<span class="delete-thumbnail-2">X</span>');
                    deleteBtn.on('click', function() {
                        var imgSrc = $(this).parent().data('src');
                        var currentUrls = $('#'+input.attr('id')).val().split(',');
                        
                        currentUrls = currentUrls.filter(function(url) {
                            return url !== imgSrc;
                        });

                        $('#'+input.attr('id')).val(currentUrls.join(',')).trigger('change');
                        $(this).parent().remove();

                        var value = $('#'+input.attr('id')).val();
                        wp.customize.instance(input.attr('id')).set(value);
                    });
    ...
    var setupDeleteEvent = function() {
        $(document).on('click', '.delete-thumbnail', function() {
            var li = $(this).closest('li');
            var imgSrc = li.data('src');
            var input = li.closest('ul').data('id');
            var currentUrls = $('#'+input).val().split(',');

            currentUrls = currentUrls.filter(function(url) {
                return url !== imgSrc;
            });

            $('#'+input).val(currentUrls.join(',')).trigger('change');

            li.remove();
            var value = $('#'+input).val();
            wp.customize.instance( input ).set(value);
        });
    };
