...
remove_action('woocommerce_after_shop_loop_item_title','tfwc_description' ,35 );
remove_action('woocommerce_after_shop_loop_item_title','tfwc_attribute_second' ,40 );
remove_action('woocommerce_after_shop_loop_item_title','tfwc_attribute_first' ,42 );
...
remove_action('woocommerce_after_shop_loop_item_title','tfwc_class_wrap_btn_open' ,55 );

remove_action( 'woocommerce_after_shop_loop_item_title', 'woocommerce_template_loop_add_to_cart', 58 );

if( class_exists('\WCBoost\Wishlist\Frontend') ) {
	remove_action( 'woocommerce_after_shop_loop_item_title', array( \WCBoost\Wishlist\Frontend::instance(), 'loop_add_to_wishlist_button' ), 60 );
}

remove_action( 'woocommerce_after_shop_loop_item_title', 'tfwc_quickview_button' , 65 );

if( class_exists('\WCBoost\ProductsCompare\Frontend') ) {
	remove_action( 'woocommerce_after_shop_loop_item_title', array( \WCBoost\ProductsCompare\Frontend::instance(), 'loop_add_to_compare_button' ), 70 );
}

remove_action( 'woocommerce_after_shop_loop_item_title', 'tfwc_class_wrap_btn_close' ,75);
