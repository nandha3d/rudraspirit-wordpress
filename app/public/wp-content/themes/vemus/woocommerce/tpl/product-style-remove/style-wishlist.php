...
if( class_exists('\WCBoost\Wishlist\Frontend') ) {
	remove_action( 'woocommerce_before_shop_loop_item', 'tf_wishlist_remove_icon_open', 5 );
	remove_action( 'woocommerce_before_shop_loop_item', array( \WCBoost\Wishlist\Frontend::instance(), 'loop_add_to_wishlist_button' ), 2 );
	remove_action( 'woocommerce_before_shop_loop_item', array( \WCBoost\Wishlist\Frontend::instance(), 'loop_add_to_wishlist_button' ), 5 );
	remove_action( 'woocommerce_before_shop_loop_item', 'tf_wishlist_remove_icon_close', 5 );
}
