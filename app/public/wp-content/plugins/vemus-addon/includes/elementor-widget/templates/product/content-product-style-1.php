...
$product_classes = 'card-product card_product--V01 type-space-35';
...
<div <?php post_class($product_classes); ?> data-availability="<?php echo esc_attr($product->get_stock_status()); ?>" data-brand="<?php echo esc_attr($product->get_attribute('pa_brand')); ?>">
    <div class="card_product-wrapper <?php echo esc_attr($wrapper_classes); ?>" style="<?php echo esc_attr($wrapper_style); ?>">
        ...
        <?php
        if( $product->get_stock_status() == "outofstock" && !empty($show_out_stock)){
            ?>
            ...
            <li>
                <?php if($product->is_type('simple')): ?>
                    <a href="<?php echo esc_url($product->add_to_cart_url()); ?>"
                        class="<?php echo esc_attr($product->is_on_backorder() || $product->is_in_stock() ? 'ajax_add_to_cart' : ''); ?> add_to_cart_button hover-tooltip tooltip-left box-icon add-to-cart"
                        data-quantity="1"
                    ...
                    >
                        ...
                        <span class="tooltip">
                            ...
                            if ( $product->is_on_backorder() ){
                                esc_html_e('Pre-order', 'vemus-addon');
                            }else{
                ...
                <?php elseif($product->is_type('external')): ?>
                    ...
                    >
                        <span class="icon icon-shop-cart"></span>
                        <span class="tooltip"><?php echo $product->get_button_text() ?></span>
                    </a>
                <?php else: ?>
                    <a href="<?php echo esc_url($product->add_to_cart_url()); ?>"
                        class="tf_quick_add_btn hover-tooltip tooltip-left box-icon add-to-cart"
                        data-bs-toggle=""
                        data-bs-target="#quickshop"
                        data-quantity="1"
                        data-product_id="<?php echo esc_attr($product->get_id()); ?>"
                    >
                        <span class="icon icon-shop-cart"></span>
                        <span class="tooltip"><?php esc_html_e('Quick Shop', 'vemus-addon'); ?></span>
                    </a>
                <?php endif; ?>
            </li>
            <li>
        ...
        if ( !empty($end_time) ) {
            ...
            if ( $seconds_remaining > 0 && empty($show_out_stock) ) {
                ?>
       ...
       <?php
        ...
        if ( !empty($second_attrs) && empty($show_out_stock) ) :
    ...
    <div class="card_product-info">
        ...
        <?php
            if ( wc_review_ratings_enabled() && $show_rating ) {
                ...
                <?php endif; ?>
        <?php } ?>
        <a href="<?php echo esc_url($product->get_permalink()); ?>" class="name-product h5 fw-normal text-white-o link text-line-clamp-2">
        ...
        <div class="cls-btn">
            <?php if($product->is_type('simple')): ?>
                <a href="<?php echo esc_url($product->add_to_cart_url()); ?>"
                    class="<?php echo esc_attr($product->is_on_backorder() || $product->is_in_stock() ? 'ajax_add_to_cart' : ''); ?> add_to_cart_button tf-btn btn-cls btn-white hover-dark hover-icon-2 w-100"
                    data-quantity="1"
                    data-product_id="<?php echo esc_attr($product->get_id()); ?>"
                >
            ...
            <?php else: ?>
                <a href="<?php echo esc_url($product->add_to_cart_url()); ?>"
                    class="tf_quick_add_btn tf-btn btn-cls btn-white hover-dark hover-icon-2 w-100"
                    data-quantity="1"
                    data-product_id="<?php echo esc_attr($product->get_id()); ?>"
                >
