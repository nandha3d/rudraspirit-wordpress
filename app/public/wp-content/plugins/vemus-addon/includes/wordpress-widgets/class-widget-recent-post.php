...
class Themesflat_Recent_Post extends WP_Widget {
    ...
    function __construct() {
        ...
        parent::__construct(
            'widget_recent_post',
            esc_html__( 'Themesflat - Recent Posts', 'vemus-addon' ),
            array(
                'classname'   => 'widget-recent-news',
                'description' => esc_html__( 'Recent Posts.', 'vemus-addon' )
            )
        );
    }
    ...
    function widget( $args, $instance ) {
        ...
        $classes[] = 'recent-news recent-blog-list';
        ...
        <ul class="<?php echo esc_attr(implode(' ', $classes)) ;?> sb-recent clearfix">  
		<?php if ( $flat_post->have_posts() ) : ?>
			<?php while ( $flat_post->have_posts() ) : $flat_post->the_post(); ?>
				<li>
                    <div class="wg-recent hover-img">
                        <?php if ( has_post_thumbnail() && $show_thumbnail == 1) : ?>
                            <a class="image img-style" href="<?php the_permalink(); ?>">
                                <?php 
                        ...
                        <div class="content">     
                            <ul class="entry_meta">
                                ...
                                <?php endif; ?>  
                                <?php if ( $show_comment ) : ?>
                                    <li><?php comments_popup_link( esc_html__( '0 Comment', 'vemus-addon' ), esc_html__(  '1 Comment', 'vemus-addon' ), esc_html__( '% Comments', 'vemus-addon' ) ); ?></li>
                                <?php endif; ?>  
                            </ul>                                           
                            <?php the_title( sprintf( '<a href="%s" class="fw-medium text-uppercase link text-line-clamp-2" rel="bookmark">', esc_url( get_permalink() ) ), '</a>' ); ?>          
                            <?php if ( $show_content ) : ?>
			...
			<?php else : ?>  
            <?php printf( '<li>%s</li>',esc_html__('Oops, category not found.', 'vemus-addon' )); ?>
		<?php endif; ?>        
        </ul>		
    ...
    function form( $instance ) {
        ...
        ?>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'vemus-addon' ); ?></label>
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>">
        </p>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php esc_html_e( 'Select Category:', 'vemus-addon' ); ?></label>
            <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'category' ) ); ?>[]">
                <option value=""<?php selected( empty( $instance['category'] ) ); ?>><?php esc_html_e( 'All', 'vemus-addon' ); ?></option>
                <?php               
      ...
      <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'ids' ) ); ?>"><?php esc_html_e( 'Get Post by IDS EX:1,2,3', 'vemus-addon' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'count' ) ); ?>"><?php esc_html_e( 'Count:', 'vemus-addon' ); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_thumbnail' ) ); ?>"><?php esc_html_e( 'Show Thumbnail ?', 'vemus-addon' ) ?></label>
        </p>  
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_content' ) ); ?>"><?php esc_html_e( 'Show Content ?', 'vemus-addon' ) ?></label>
        </p>       
        <p>
            <input class="checkbox" type="checkbox" <?php checked( $show_date ); ?> id="<?php echo esc_attr( $this->get_field_id( 'show_date' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'show_date' ) ); ?>" />
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_date' ) ); ?>"><?php esc_html_e( 'Show Date ?', 'vemus-addon' ); ?></label>
        </p>
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_comment' ) ); ?>"><?php esc_html_e( 'Show Comment ?', 'vemus-addon' ); ?></label>
        </p>
    <?php
    }
}
